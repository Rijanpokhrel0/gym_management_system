/* ==========================================================================
   FITPULSE - TRAINER DIET PLANS MODULE
   ========================================================================== */
...
(function () {
  ...
  async function loadTrainerDiets() {
    try {
      ...
      const grid = $('trainer-diets-grid');
      if (grid) {
        grid.innerHTML = (d.plans || []).map(trainerDietCard).join('') || emptyState('No diet plans yet. Create one.');
      }
    } catch (err) {
      const grid = $('trainer-diets-grid');
  ...
  // Global gm actions
  window.gm = window.gm || {};
  window.gm.editDiet = (id) => api('api/trainer/diets.php').then((d) => openDietModal((d.plans || []).find((p) => String(p.id) === String(id)))).catch((e) => toast(e.message, 'error'));
  window.gm.assignDiet = (id) => openAssignDiet(id);
  window.gm.deleteDiet = async (id) => {
    if (!confirm('Delete this diet plan?')) return;
    try {
      await api('api/trainer/diets.php', { method: 'DELETE', body: { id } });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const btnAdd = $('btn-trainer-add-diet');
    ...
    if (formDiet) {
      formDiet.addEventListener('submit', async (e) => {
        ...
        try {
          await api('api/trainer/diets.php', { method: id ? 'PUT' : 'POST', body: id ? { id, ...payload } : payload });
    ...
    if (formAssign) {
      formAssign.addEventListener('submit', async (e) => {
        ...
        if (!added.length && !removed.length) { closeModal('modal-assign-diet'); toast('No changes.'); return; }
        try {
  ...
  window.TrainerApp.registerLoader('tab-trainer-diets', loadTrainerDiets);
})();
