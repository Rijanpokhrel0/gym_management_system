<?php
/**
 * Admin: assign / unassign diet plans to members (own gym).
...
$stmt = db()->prepare('SELECT id FROM diet_plans WHERE id = ? AND admin_id = ?');
...
switch ($method) {
    case 'GET':
        $stmt = db()->prepare('SELECT da.user_id, u.name, u.email, u.member_code, da.assigned_at
    ...
    case 'DELETE':
        ...
        db()->prepare('DELETE FROM diet_assignments WHERE plan_id = ? AND user_id = ?')->execute([$planId, $userId]);
