/* ==========================================================================
   FITPULSE - ADMIN WORKOUT PLANS MODULE
   ========================================================================== */
...
(function () {
  ...
  async function loadAdminWorkouts() {
    try {
      ...
      const grid = $('admin-workouts-grid');
      if (grid) {
        grid.innerHTML = (d.plans || []).map(adminWorkoutCard).join('') || emptyState('No workout plans yet. Create one.');
      }
    } catch (err) {
      const grid = $('admin-workouts-grid');
  ...
  // Global gm actions
  window.gm = window.gm || {};
  window.gm.editWorkout = (id) => api('api/admin/workouts.php').then((d) => openWorkoutModal((d.plans || []).find((p) => String(p.id) === String(id)))).catch((e) => toast(e.message, 'error'));
  window.gm.assignWorkout = (id) => openAssignWorkout(id);
  window.gm.deleteWorkout = async (id) => {
    if (!confirm('Delete this workout plan?')) return;
    try {
      await api('api/admin/workouts.php', { method: 'DELETE', body: { id } });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (formWorkout) {
      formWorkout.addEventListener('submit', async (e) => {
        ...
        try {
          await api('api/admin/workouts.php', { method: id ? 'PUT' : 'POST', body: id ? { id, ...payload } : payload });
  ...
  window.AdminApp.registerLoader('tab-admin-workouts', loadAdminWorkouts);
})();
